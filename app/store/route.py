...
from flask import render_template,request,abort,session,g,redirect,url_for,jsonify
...
@store_bp.route("/register_pay",methods={"GET","POST"})
def register_pay():
    if request.args.get("data") == '1':
        return jsonify({"response":''.join(choice(ascii_letters) for i in range(10))})
